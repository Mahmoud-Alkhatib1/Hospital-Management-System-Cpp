...
public:
	...
	{
		return to_string(Time.Hours) + ":" + to_string(Time.Min);
	}*/
	static string TimeToString(stTime Time) {
		string HoursStr = (Time.Hours < 10 ? "0" + to_string(Time.Hours) : to_string(Time.Hours));
		string MinStr = (Time.Min < 10 ? "0" + to_string(Time.Min) : to_string(Time.Min));
	...
	static string DayShortName2(short DayOfWeekOrder)
	{
		string arrDayNames[7] = { "Sunday","Monday","Tuesday"
